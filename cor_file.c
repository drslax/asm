...
/*   Updated: 2020/01/27 06:19:23 by slyazid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		{
// 			if (inst->args[index]->type != T_IND)
// 				value |= (inst->args[index]->type & 0xFF) << (6 - 2 * index);
// 			printf("value = %d | inst->type = %s\n", value, inst->args[index]->name);
// 		}
// 		index += 1;
// 	}
// 	// printf("size = %d\n", inst->size);
		...
		if (inst->args[index]->name)
		{
			...
			else
			{
				...
				if (inst->args[index]->type == T_DIR && type_size(inst->args[index]->type, inst->id) == 2)
					...
					ft_write_short(filedesc, ft_atoi(inst->args[index]->name));
				else
				{
